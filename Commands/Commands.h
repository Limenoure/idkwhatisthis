...
5 DEVELOPER
4 ADMINISTRATOR
3 MODERATOR D:\davin\EternityV2
2 VIP
1 CHEATER
0 PLAYER
*/
...
public:
	...
	static void Command() {
		...
		commandMap["/sellfish"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SELLFISH(peer); }, 0 };
		...
		commandMap["/setclist"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETCLIST(peer, param); }, 9 };
		commandMap["/clist"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::CLIST(peer); }, 9 };
		//commandMap["/porte_arriere"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SERVER_STAT(peer); }, 9 };
		commandMap["/stop_server"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::STOP_SERVER(peer); }, 9 };
		commandMap["/resetlock"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::RESET_LOCK(peer); }, 9 };
		commandMap["/setevent"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETEVENT(peer); }, 9 };
		...
		commandMap["/maintenance"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::MAINTENANCE(peer); }, 9 };
		commandMap["/setrcode"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETRCODE(peer); }, 9 };
		commandMap["/setnewget"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETNEWGET(peer); }, 9 };
		commandMap["/setgiveaway"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETGIVEAWAY(peer); }, 9 };
		//commandMap["/setworld"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::SETWORLD(peer); }, 9 };
		...
		commandMap["/checkpass"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::CHECK_PASS(peer, param); }, 9 };
		...
		commandMap["/takeworld"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::TAKEWORLD(peer); }, 9 };
		commandMap["/clearworld"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::CLEAR_WORLD(peer); }, 9 };
		...
		commandMap["/gtoken"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::GTOKEN(peer, param); }, 9 };
		commandMap["/gseal"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::GSEAL(peer, param); }, 9 };
		...
		commandMap["/removelogs"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::REMOVE_LOGS(peer, param); }, 9 };
		commandMap["/ggems"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::GGEMS(peer, param); }, 9 };
		commandMap["/glevel"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::GLEVEL(peer, param); }, 9 };
		...
		commandMap["/dropall"] = { [](ENetPeer* peer, const std::string& param, int level) { Commands::DROPALL(peer); }, 3 };
...
private:
	static void HELP(ENetPeer* peer) {
		VarList::OnConsoleMessage(peer, a + "`o>> `5Player Commands`o: /msg /status /bc /sb /sdb /stats /top /ignore /time /who /me /radio /mods /uba /r /go /rgo /hub /gor /pull /kick /ban /kickall /wave /dance /love /sleep /facepalm /fp /smh /yes /no /hideui /renderworld /lol /troll /wink /shower /cry /omg /idk /shrug /trade /pb /furious /rolleyes /wl /unaccess /report /reportbug /rate /foldarms /fa /stubborn /fold /fc /redeemcode /secureaccount /gc /gwarp /guild /roles /fav /favlist /home /sethome /gethome /dab /sassy /dance2 /march /favrm /grumpy /shy /janeway /growpass /respawn /tradescan /tips /event /vips /deposit /modrules /csnrules /rules /news /howgay /khodam /wipeworlds /fixmygems /spotify /a /clearchat /serverinfo /pets /shop /buy /casinolist /reqverifycsn " + a + (pInfo(peer)->supp >= 2 ? "/hidestatus " : "") + (Role::Cheater(peer) ? "/cheats /c <cheater chat> " : "")
			+ (Role::Vip(peer) ? "\n`o>> `1[VIP] Commands`o: /surgerystats /vhelp /v <chat> /searchuser <player name> /info <player name> (shows inventory) /hidestatus /warp <world> /warpto <player name> /vsb <text>" : "")
			+ (Role::Moderator(peer) ? "\n`o>> `#@Moderator Commands`o: /warpto <player name> /destructo /noclip /fixworld /searchuser <player name> /summon <player name> /freeze <player name> /info <player name> /p <id> /warp <player name> /banworld /nuke <time> <reason> /mute <player name> <time> <reason> /curse <player name> <time> <reason> /unmute <player name> /uncurse <player name> /hide /radio2 /togglemods /skin /invis /copyset <player name> /msb <text> /m <text> /magic /nohands /nobody /unequip /warn <player name> <text> /dropall /eff <id> /pban <player name> /note <player name> <text> /notes <player name>" : "")
			...
			+ (Role::Developer(peer) ? "\n`o>> `9@Developer Commands`o: /gsm <text> /blacklist <player name> /online /saveworld /spk <player name> <text> /asb <text> /removedrop /spl (spawn public lava) /rpl (remove public lava) /growall /infoex <player name> /banrid <player name> /banip <player name> /checkplaymods <player name>" : "")
			+ (Role::God(peer) ? "\n`o>>`9@God Commands`o: ....." : "")
			...
			+ (Role::Owner(peer) ? "\n`o>>`6@Owner Server Commands`o: /reset <player name> /takeworld /osb <text> /unsafeedit <action> <radius> <item_id> /weather <id> /clearworld /spawnevent /ggxp <count> /gtoken <count> /gseal <count> /reset_rolequest /verifycsn /removereqcsn /ipcheck <player name> /removelogs <player name>" : "")
			+ (Role::Clist(pInfo(peer)->tankIDName) ? "\n`o>> `2@Creator Commands`o: /getitem <item name> /addpbuy <id> <price> /resetlock <reset lock in all player> /give <player name> <item name> <quantity> /addpscan <id> <price> /deleteworld <name world> /deleteacc <player name> /setevent /wotd /wotw /maintenance /checkpass <player name> /grole <player name> /gassets <player name> /gwrench <player name> /setrcode /setnewget /setworldmenu <name world> /setgiveaway /setworld /tfbank <player name> /removebank <player name> /edititem <itemid> /editstorestock /additemtonpc <itemid>" : ""));
	}
	...
	static void BUY(ENetPeer* peer) {
		...
		VarList::OnConsoleMessage(peer, "This features have been disabled for a while.");
		pInfo(peer)->Has_In = true;
	}
	static void SELLFISH(ENetPeer* peer) {
		string fish_dialog = "";
		...
		if (fish_dialog.empty()) fish_dialog = "\nadd_textbox|`oYou don't have any fish to sell!|left|";
		VarList::OnDialogRequest(peer, SetColor(peer) + "set_default_color|\nadd_label_with_icon|big|`wSell Fish Machine|left|10374|\nadd_spacer|small|\nadd_smaltext|`oThe use of this Machine is to sell the fish you have.|left|\nadd_smalltext|`oList of fish that can be Sell:|left|\nadd_spacer|small|" + fish_dialog + "|\nend_dialog|sell_fish|No, Thanks|Sell Fish|");
	}
	...
	static void RESPAWN(ENetPeer* peer) {
		if (pInfo(peer)->WarnRespawn == 5) {// Anti Spam Respawn By nozelim
	...
	static void GET_ITEMS(ENetPeer* peer, std::string cmd) {
		...
		for (int i_ = 0; i_ < items.size(); i_++) {
			...
			if (pInfo(peer)->tankIDName != "nozelim") {
				if (i_ == 9506) continue;
			}
			...
			try {
				if (ss >> itemId >> quantity) {
					...
					if (quantity < 1 || quantity > 5000) {
						VarList::OnConsoleMessage(peer, "`8>> `4Oops: `8Minimum is 1 and the maximals is 5000 items droppeds!");
						return;
					}
	...
	static void FIND_ITEM(ENetPeer* peer) {
		...
		for (const auto& item : Environment()->find_item_list) {
			if (pInfo(peer)->tankIDName != "nozelim") if (item.first == 9506) continue;
	...
	static void SPK(ENetPeer* peer, std::string cmd) {
		if (cmd.empty()) {
			VarList::OnConsoleMessage(peer, "`oUsage: /spk <text>");
			return;
		}
		std::istringstream ss(cmd);
	...
	static void ASB(ENetPeer* peer, std::string cmd) {
		if (cmd.empty()) {
			VarList::OnConsoleMessage(peer, "`oUsage: /asb <text>");
			return;
		}
		for (ENetPeer* cp_ = server->peers; cp_ < &server->peers[server->peerCount]; ++cp_) {
			if (cp_->state != ENET_PEER_STATE_CONNECTED or cp_->data == NULL) continue;
