...
string get_player_tag(string name) {
	...
	if (ifs.is_open()) {
		json j;
		ifs >> j;
		if (j["Role.Owner_Server"] == true) return "`6@" + j["name"].get<string>();
		if (j["Role.Developer"] == true) return "`9@" + j["name"].get<string>();
		...
		for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
			...
			for (GuildMember search_member : guild_information->guild_members) {
				if (to_lower(search_member.member_name) == to_lower(pInfo(currentPeer)->tankIDName)) {
					string display_color = (pInfo(currentPeer)->Role.Owner_Server ? "`6@" : pInfo(currentPeer)->Role.Developer ? "`9@" : pInfo(currentPeer)->Role.Administrator ? "`e@" : pInfo(currentPeer)->Role.Moderator ? "`#@" : "`o");
