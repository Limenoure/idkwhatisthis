...
#include <string>
#include <WorldInfo.h>
#include "GrowTavern/Commands/Commands.h"
...
private:
	...
	static void Enter_Game(ENetPeer* peer, std::string cch) {
		if (pInfo(peer)->bypass and !pInfo(peer)->in_enter_game) {
			...
			if (Role::Moderator(peer)) VarList::OnConsoleMessage(peer, "`oYou are " + a + (Role::Owner(peer) ? "`6@Owner Server" : Role::Developer(peer) ? "`9@Developer" : Role::Administrator(peer) ? "`6@Administrator" : Role::Moderator(peer) ? "`#@Moderator" : "") + "``, Welcome to " + Environment()->server_name + " don't forget to read /modrules before Playing.");
	...
	static void respawn(ENetPeer* peer, std::string cch) {
		...
		if (pInfo(peer)->WarnRespawn >= 25) {// Anti Spam Respawn By nozelim
